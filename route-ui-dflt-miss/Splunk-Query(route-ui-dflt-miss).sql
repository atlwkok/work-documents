-- Splunk query
| dbxquery query="SELECT * FROM ( SELECT 'IND' AS WHSE, CASE WHEN COUNT(*) = '0' THEN 'MISSING DFLT STATIC ROUTE VALUE' ELSE 'OK' END AS STATUS FROM ( SELECT STATIC_ROUTE_ID, PROFILE_ID FROM default_routing.RTG_STATIC_ROUTE  WHERE STATIC_ROUTE_ID = 'DFLT' AND PROFILE_ID = 'ORL_Org_Profile' )A  )B WHERE STATUS <> 'OK'" connection="MAPRD_NEW"
...
FROM   (SELECT 'IND' AS 'WHSE',
               CASE
                 WHEN Count(*) = '0' THEN 'MISSING DFLT STATIC ROUTE VALUE'
                 ELSE 'OK'
               END   AS 'STATUS'
        FROM   (SELECT static_route_id,
                       profile_id
