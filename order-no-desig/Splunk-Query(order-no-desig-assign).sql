-- Splunk query
| dbxquery query="SELECT DISTINCT O.org_id,
                O.original_order_id,
                O.order_type,
...
WHERE  1 = 1
       ...
       AND O.minimum_status < '7800'
       AND OLI.status NOT IN ( 'CANCELLED' )
       AND O.created_timestamp <= Now() - INTERVAL 15 minute
       AND ( O.designated_shipment_id NOT IN (SELECT DISTINCT shipment_id
                                              FROM
             default_shipment.shp_shipment S
