...
insert into app.cronaction (
	...
	select 
		c.crontaskid,
		cat.CronActionTypeId,
		'{"EmailTextOnError":"tbd","TaskName":"DB Alert: Orders Deselected Waving","StateMachine":"wms-tools-state-machine-db-alert","Lambda":"wms-tools-lambda-db-alert","RequestType":"db-alert-orders-deselected-waving"}',
		'sysuser', 'sysuser'
	from 
		app.cronactiontype cat, app.crontasks c
	where
		c.description = 'DB Alert: Orders Deselected Waving'
		and not exists (select 1 from app.cronaction c2 where c2.crontaskid = c.crontaskid)
		and cat.description = 'NONE';