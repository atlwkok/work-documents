-- Splunk query - DB Alert: MAWM -In Packing Olpns > 5 Minutes
| dbxquery query="SELECT ORG_ID, OLPN_ID, STATUS, ORDER_ID, ORDER_TYPE
, ORDER_PLANNING_RUN_ID, SHIPMENT_ID, ORDER_PIPELINE_ID
, UPDATED_BY , UPDATED_TIMESTAMP
FROM default_pickpack.PPK_OLPN
WHERE 1=1
AND STATUS = '7100'
AND ORDER_TYPE = 'Standard'
AND UPDATED_TIMESTAMP <= NOW() - INTERVAL 5 MINUTE" connection="MAPRD_NEW"
...
AND STATUS = '7100'
AND ORDER_TYPE = 'Standard'
AND UPDATED_TIMESTAMP <= NOW() - INTERVAL 5 MINUTE


-- Flattening SQL query to single line (paste and copy from chrome search bar)
SELECT ORG_ID,          OLPN_ID,          STATUS,          ORDER_ID,          ORDER_TYPE,          ORDER_PLANNING_RUN_ID,          SHIPMENT_ID,          ORDER_PIPELINE_ID,          UPDATED_BY,          UPDATED_TIMESTAMP FROM default_pickpack.PPK_OLPN WHERE 1=1 AND STATUS = '7100' AND ORDER_TYPE = 'Standard' AND UPDATED_TIMESTAMP <= NOW() - INTERVAL 5 MINUTE 
