...
| dbxquery query="SELECT O.ORG_ID, O.ORIGINAL_ORDER_ID, O.CREATED_TIMESTAMP, O.ORDER_TYPE, 
 CASE WHEN UPPER(ORDER_TYPE) = 'STANDARD' THEN O.EXT_ROUTE_ID else NULL END AS ROUTE
 , CONCAT(O.MAXIMUM_STATUS,'-',DS.DESCRIPTION) MAX_STATUS
 from default_dcorder.DCO_ORIGINAL_ORDER O 
 JOIN default_dcorder.DCO_ORDER_STATUS DS ON DS.ORDER_STATUS_ID = O.MAXIMUM_STATUS
 JOIN default_dcorder.DCO_ORDER_STATUS DS2 ON DS2.ORDER_STATUS_ID = O.MINIMUM_STATUS
 WHERE 1=1
 AND O.MAXIMUM_STATUS < 8000
 AND O.CREATED_TIMESTAMP < NOW() - INTERVAL 7 DAY
 AND O.ORG_ID = 'ABQ'" connection="MAPRD_NEW"
...
SELECT O.ORG_ID, 
        ...
        O.CREATED_TIMESTAMP, 
        O.ORDER_TYPE, 
        CASE WHEN UPPER(ORDER_TYPE) = 'STANDARD' 
                THEN O.EXT_ROUTE_ID 
                else NULL 
        END AS 'ROUTE', 
        CONCAT(O.MAXIMUM_STATUS,'-',DS.DESCRIPTION) 'MAX_STATUS'
